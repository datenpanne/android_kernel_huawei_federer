...
static int spk_hs_hpa_off_contol_en = 0; //if read "qcom,spk-hs-turnoff-hph-bypass" from huawei-audio.dtsi,then set it
...
static void wcd_mbhc_set_and_turnoff_hph_padac(struct wcd_mbhc *mbhc)
{
	...
	* removal event to sync-up PA's state */
	...
	if(!spk_hs_hpa_off_contol_en){
		snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_RX_HPH_CNP_EN,
			    0x30, 0x00);
	}else{
		if(mbhc->mbhc_cb && mbhc->mbhc_cb->get_spk_pa_swtich_status){
			if(!mbhc->mbhc_cb->get_spk_pa_swtich_status()){
			...
			ad_loge("%s:mbhc->mbhc_cb or mbhc->mbhc_cb->get_spk_pa_swtich_status is null\n", __func__);
		}
	}
	usleep_range(wg_time * 1000, wg_time * 1000 + 50);
}
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (mbhc->support_iphone_headset && (mbhc->current_plug != MBHC_PLUG_TYPE_HEADPHONE)) {
			ad_logn("%s: force micbias_enable(2.8V) for support ipnone headset.\n", __func__);
		      ...
		      bool impedance_det_en)
{
	...
	const char *mbhc_iphone_headset = "huawei,support-iphone-headset";
	...
	mbhc->support_iphone_headset =
		(of_property_read_bool(card->dev->of_node, mbhc_iphone_headset) ? true : false);
	...
	spk_hs_hpa_off_contol_en = (of_property_read_bool(card->dev->of_node, spk_hs_hph_off_bypass) ? 1 : 0);
	ad_logn("%s: spk_hs_hpa_off_contol_en = %d\n", __func__, spk_hs_hpa_off_contol_en);
