...
/* < DTS2016021502632  liuanqi 20160316 begin */
enum led_brightness  msm_led_torch_brightness_get(struct led_classdev *led_cdev)
{
	enum led_brightness value=0;
	int rc=0;

	if( fctrl.func_tbl->torch_led_get_status )
		rc = fctrl.func_tbl->torch_led_get_status(&fctrl);
	if( rc<0 ){
		pr_err("Failed to read torch status. rc = %d\n", rc);
	}else if( rc==1){
		value=127;
	}
...
/* DTS2016021502632  liuanqi 20160316 end > */
...
int msm_torch_lm3642_led_on(struct msm_led_flash_ctrl_t *fctrl)
{
    ...
    /* < DTS2015042906355 y00294389 20150525 begin */
	/* < DTS2014110404271  yangzhenxi 20141104 begin */
	if (fctrl->flash_i2c_client && fctrl->reg_setting) {
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_write_table(
			fctrl->flash_i2c_client,
	...
	gpio_set_value_cansleep(
		...
		gpio_num[SENSOR_GPIO_FL_NOW],
		GPIO_OUT_HIGH);
	/* DTS2014110404271  yangzhenxi 20141104 end >*/
	/* DTS2015042906355 y00294389 20150525 end > */
...
/* < DTS2016021502632  liuanqi 20160316 begin */
int msm_torch_lm3642_led_get_status(struct msm_led_flash_ctrl_t *fctrl )
{
	int32_t rc = 0;
	int32_t status=0;
	uint16_t reg_value = 0;
	...
	msm_flash_clear_err_and_unlock(fctrl);
		...
		rc = fctrl->flash_i2c_client->i2c_func_tbl->i2c_read(
			fctrl->flash_i2c_client,ENABLE_REGISTER,&reg_value, MSM_CAMERA_I2C_BYTE_DATA);
		if(rc < 0){
			pr_err("%s: FLASHCHIP READ I2C error!\n", __func__);
		}
		if(reg_value==(MODE_BIT_TORCH | EBABLE_BIT_IVFM)){
			status=1;//led_torch is on
		}
		return status;
}
/* DTS2016021502632  liuanqi 20160316 end > */
...
static struct msm_flash_fn_t lm3642_func_tbl = {
	...
	/* < DTS2016021502632  liuanqi 20160316 begin */
	.torch_led_get_status=msm_torch_lm3642_led_get_status,
	/* DTS2016021502632  liuanqi 20160316 end > */
	/*<DTS2015071103112 w00182304 20150711 begin*/
