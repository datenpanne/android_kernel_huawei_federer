		...
		unsigned int bus_width, u8 *ext_csd)
{
    ...
    /* added for sandisk powerclass setting*/
    pr_info("[HW]:%s, %d: pwrclass_val=0x%x\n", __func__, __LINE__, pwrclass_val);
    /*change default power class to 0x08 just for sandisk 32GB and 64GB emmc*/
    if((0 == strcmp(card->cid.prod_name, "SDW16G")) && (0x45 == card->cid.manfid))
    {
        if(0x0d == pwrclass_val)
        {
            pr_info("[HW]:%s, %d: pwrclass_val=0x%x and set to 0x08\n", __func__, __LINE__, pwrclass_val);
            pwrclass_val = 0x08;
        }
    }
