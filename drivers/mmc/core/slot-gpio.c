...
static irqreturn_t mmc_gpio_cd_irqt(int irq, void *dev_id)
{
	...
	if (status ^ ctx->status) {
		...
		if (host->sd_irq_wake_lock_flag) {
			if (wake_lock_active(&host->sd_irq_wake_lock))
				wake_unlock(&host->sd_irq_wake_lock);
			wake_lock_timeout(&host->sd_irq_wake_lock, 5 * HZ);
			pr_info("%s: sd irq wake lock 5s\n", mmc_hostname(host));
		}
#endif
