...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if(!strcmp(mmc_hostname(msm_host->mmc), "mmc1")) {
		if (of_get_property(pdev->dev.of_node, "huawei,support-uhs", NULL)) {
			msm_host->mmc->caps &= ~(MMC_CAP_UHS_SDR104 | MMC_CAP_UHS_DDR50);
		} else {
...
#ifdef CONFIG_MMC_BLOCK_DEFERRED_RESUME
	if(!strcmp(mmc_hostname(msm_host->mmc), "mmc1")) {
		if (of_get_property(pdev->dev.of_node, "huawei,sd-irq-wake-lock", NULL)) {
			msm_host->mmc->sd_irq_wake_lock_flag = true;
		} else {
			msm_host->mmc->sd_irq_wake_lock_flag = false;
		}
	}
#endif
