...
static ssize_t led_rg_lcd_color_temperature_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
	struct mdss_panel_data *pdata;
	struct fb_info *fbi = dev_get_drvdata(dev);
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d,%d,%d\n", pdata->panel_info.led_rg_para1, pdata->panel_info.led_rg_para2,
							led_ct_table [0], led_ct_table [1], led_ct_table [2]);
}

static ssize_t led_rg_lcd_color_temperature_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
	struct mdss_panel_data *pdata;
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	uint32_t led_rg_para[100];
	char *token, *cur;
	int i = 0;
	...
	token = strsep(&cur, ",");
	while (token) {
		led_rg_para[i++] = simple_strtol(token, NULL, 0);
		token = strsep(&cur, ",");
	}

	pdata->panel_info.led_rg_para1= led_rg_para[0];
	pdata->panel_info.led_rg_para2= led_rg_para[1];
...
static DEVICE_ATTR(led_rg_lcd_color_temperature, S_IRUGO|S_IWUSR, led_rg_lcd_color_temperature_show, led_rg_lcd_color_temperature_store);
...
void mdss_mdp_set_attenuate_blue_coff_overlay(struct mdss_panel_info *panel_info)
{
	if( NULL == panel_info) {
		pr_info("panel_info is NULL\n");
	}
	mdss_mdp_set_attenuate_blue_coff(panel_info->attenuate_blue_coff);
}
...
(color temprature become invalid after reboot)*/
	case mdp_op_pre_pcc_cfg:
		mdss_mdp_set_attenuate_blue_coff_overlay(mfd->panel_info);
