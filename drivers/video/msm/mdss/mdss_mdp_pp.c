...
/* < DTS2015082001462 fwx295956 20150820 begin */
    struct mdp_pcc_cfg_data saved_pcc_disp_cfg[MDSS_BLOCK_DISP_NUM];
/* DTS2015082001462 fwx295956 20150820 end > */
};
...
static int g_attenuate_blue_flag =0;
static unsigned int g_attenuate_blue_coff=0; /*0~99%*/

static void mdss_mdp_attenuate_blue_extra(int disp_num);
...
/* DTS2015082001462 fwx295956 20150820 end > */
/*<  DTS2016021403416  feishilin 20160217 begin */
		mdss_mdp_attenuate_blue_extra(disp_num);
/*  DTS2016021403416  feishilin 20160217 end >*/
...
/* < DTS2015082001462 fwx295956 20150820 begin */
/* < DTS2015090808787 fwx295956 20150908 begin */
/* < DTS2015122203508  feishilin 20151222 begin */
//delete this, refresh by application(it will be die if panel is command mode)
/*  DTS2015122203508  feishilin 20151222 end >*/
int mdss_mdp_pp_set_comform_coff(unsigned int coff_val, unsigned int update_flag)
{
	...
	if(g_attenuation_coff == 100){
		mdss_mdp_attenuate_blue_enable_bit(0, COMFORM_COFF_MASK);
		g_attenuation_coff = 0;
	}
	else
	{
		mdss_mdp_attenuate_blue_enable_bit(1, COMFORM_COFF_MASK);
	}
	/*  DTS2016021403416  feishilin 20160217 end >*/
...
/*<  DTS2016021403416  feishilin 20160217 begin */
void mdss_mdp_attenuate_blue_enable_bit(unsigned int enable_flag, unsigned int bit)
{
	if(enable_flag){
		g_attenuate_blue_flag |= bit;
	}
	else{
		g_attenuate_blue_flag &= ~bit;
	}
...
void mdss_mdp_set_attenuate_blue_coff(unsigned int coff)
{
	g_attenuate_blue_coff = coff;
}

static void  mdss_mdp_attenuate_blue_extra(int disp_num)
{
	if(g_attenuate_blue_flag && g_attenuate_blue_coff)
	{
		unsigned long temp_blue;
		pr_info(": extra coff=%d old r=%x, g=%x, b=%x\n", g_attenuate_blue_coff,
                            mdss_pp_res->pcc_disp_cfg[disp_num].r.r,
                            mdss_pp_res->pcc_disp_cfg[disp_num].g.g,
                            mdss_pp_res->pcc_disp_cfg[disp_num].b.b);
		temp_blue = (unsigned)mdss_pp_res->pcc_disp_cfg[disp_num].b.b;
		temp_blue = ((temp_blue*g_attenuate_blue_coff)+50)/100;
		mdss_pp_res->pcc_disp_cfg[disp_num].b.b = (unsigned int)temp_blue;
		pr_info(": extra coff=%d new r=%x, g=%x, b=%x\n", g_attenuate_blue_coff,
                            mdss_pp_res->pcc_disp_cfg[disp_num].r.r,
                            mdss_pp_res->pcc_disp_cfg[disp_num].g.g,
                            mdss_pp_res->pcc_disp_cfg[disp_num].b.b);
	}
