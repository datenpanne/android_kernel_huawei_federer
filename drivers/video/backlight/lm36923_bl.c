...
int lm36923_chip_init(struct lm36923_chip_data *pchip)
{
	...
	if (BRT_REG_ONLY_MODE == lm36923_ctrl_mode) {
		...
		ret = regmap_update_bits(pchip->regmap, REG_BRT_CTR, MASK_CHANGE_MODE,
			LM36923_BLED_MODE_LINEAR|BRT_MUL_RAMP|LM36923_RAMP_EN|RAMP_RATE_1|BL_ADJ_HIGHT);
	}
	...
	ret = regmap_update_bits(pchip->regmap, REG_PWM_CTR, MASK_CHANGE_MODE,
		LM36923_PWM_SAMPLE_RATE_240|LM36923_PWM_IN_POL_H|LM36923_PWM_HYS_NONE|LM36923_PWM_FILTER_200);
	if (ret < 0)
		goto out;
