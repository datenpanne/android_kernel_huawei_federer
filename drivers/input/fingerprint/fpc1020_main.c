...
/* <DTS2015042903888 LiuQiang   20150429 begin */
/* <DTS2015051105241 cheyufeng  20150511 begin */
...
/* < DTS2015032001259 caiying/00214377 20150320 begin */
#include "fpc1020_navlib.h"
/* DTS2015032001259 caiying/00214377 20150320 end > */
...
/* < DTS2015040704579 caiying/00214377 20150407 begin */
#define TEST_DEADPIXELS_CAPTURE_TIMEOUT (3000*HZ/1000)
/* DTS2015040704579 caiying/00214377 20150407 end > */
...
static int fpc1020_module_id_init(struct device *dev, fpc1020_data_t *fpc1020);
...
/*< DTS2015013104641 lijianzhao 20150131 begin */
static ssize_t fingerprint_chip_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    return scnprintf(buf,
            ...
            fpc1020->chip.revision, fpc1020->module_vendor);
...
/* -------------------------------------------------------------------- */
/* < DTS2015053003470 zhaoyuxia 20150530 begin */
/* < DTS2015063008218 zhaoyuxia 20150701 begin */
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    ...
    if (*blank == FB_BLANK_UNBLANK) {
            ...
            /* < DTS2015060305033 zhaoyuxia 20150609 begin */
            /* remove spi ctrl work */
            /* DTS2015060305033 zhaoyuxia 20150609 end > */
            /*suspend device*/
            switch(event) {
            case FB_EARLY_EVENT_BLANK:
...
#endif
/* DTS2015063008218 zhaoyuxia 20150701 end > */
/* DTS2015053003470 zhaoyuxia 20150530 end > */
...
/* DTS2015032001259 caiying/00214377 20150320 end > */
    ...
    error = fpc1020_module_id_init(&spi->dev, fpc1020);
    if(error)
	 goto err;
...
static int fpc1020_module_id_init(struct device *dev, fpc1020_data_t *fpc1020)
{
	int error = 0;
	int vendor_value;
	struct pinctrl *module_id_pinctrl;
        struct pinctrl_state *p_state, *n_state, *d_state;

        memset(fpc1020->module_vendor, 0, sizeof(fpc1020->module_vendor));
	...
	if (gpio_is_valid(fpc1020->moduleID_gpio)) {
            ...
            module_id_pinctrl = devm_pinctrl_get(dev);
            if(IS_ERR(module_id_pinctrl)) {
                fpc_log_err("get pinctrl handler failed!\n");
                return -1;
            }

            d_state = pinctrl_lookup_state(module_id_pinctrl, "fpc1020_module_id_default");
            if(IS_ERR(d_state)){
                fpc_log_err("get pinctrl default state failed!\n");
                return -1;
            }
            p_state = pinctrl_lookup_state(module_id_pinctrl, "fpc1020_module_id_positive");
            if(IS_ERR(p_state)){
                fpc_log_err("get pinctrl positive state failed!\n");
                return -1;
            }
            n_state = pinctrl_lookup_state(module_id_pinctrl, "fpc1020_module_id_negative");
            if(IS_ERR(n_state)){
                fpc_log_err("get pinctrl negative state failed!\n");
                return -1;
            }

            error = pinctrl_select_state(module_id_pinctrl, p_state);

            error = gpio_direction_input(fpc1020->moduleID_gpio);

            mdelay(10);

            vendor_value = gpio_get_value_cansleep(fpc1020->moduleID_gpio);	/* PULL-UP State */

            error = pinctrl_select_state(module_id_pinctrl, n_state);

            error = gpio_direction_input(fpc1020->moduleID_gpio);

            mdelay(10);

            vendor_value = vendor_value<<1;	/* left-shift-one-bit */

            vendor_value |= gpio_get_value_cansleep(fpc1020->moduleID_gpio);	/* PULL-DOWN State */

            switch(vendor_value){
            case 0:
                strncpy(fpc1020->module_vendor, "LO", strlen("LO"));	/* O-Film */
                break;
            case 2:
                strncpy(fpc1020->module_vendor, "QT", strlen("QT"));	/* Q-Tech */
                break;
            case 3:
                strncpy(fpc1020->module_vendor, "CT", strlen("CT"));	/* CT */
                break;
            default:
                strncpy(fpc1020->module_vendor, "NN", strlen("NN"));	/* Unknown Vendor */
                break;
            }

            error = pinctrl_select_state(module_id_pinctrl, d_state);		/* NO-PULL state */

            error = gpio_direction_input(fpc1020->moduleID_gpio);

            if (error) {
                fpc_log_err("gpio_direction_input (moduleID_gpio) failed\n");
                devm_pinctrl_put(module_id_pinctrl);
                return error;
            }
    }
...
/* DTS2015053003470 zhaoyuxia 20150530 end > */
/* DTS2015013104641 lijianzhao 20150131 end >*/
...
/* -------------------------------------------------------------------- */
/* < DTS2015090705390 tongyongjiao/wx205235 20150907 begin */
...
int fpc1020_check_deadpixels_in_detect_zone(fpc1020_data_t *fpc1020, int index)
{
    ...
    switch (fpc1020->chip.type) {
        case FPC1020_CHIP_1020A:
            ...
            ySubareaNum = 3;
            break;
        /* <DTS2015051201551 LiuQiang 20150512 begin */
        ...
        case FPC1020_CHIP_1021F:
            ...
            ySubareaNum = 3;
            break;
        /* DTS2015051201551 LiuQiang 20150512 end >*/
        case FPC1020_CHIP_1150A:
            ...
            break;
        /* < DTS2015031001664 caiying/00214377 20150310 begin */
        /* < DTS2015063006217 caiying/00214377 20150630 begin */
        /* < DTS2015081000631 tongyongjiao/wx205235 20150810 begin */
        ...
        case FPC1020_CHIP_1140C:
            ...
            ySubareaNum = 4;
            break;
        /* DTS2015081000631 tongyongjiao/wx205235 20150810 end > */
        /* DTS2015063006217 caiying/00214377 20150630 end > */
        /* DTS2015031001664 caiying/00214377 20150310 end > */
        default:
            error = FPC_1020_DEADPIXEL_DETECT_ZONE_HW_ID_ERROR;
            goto out;
    }
    /* < DTS2015081500317 tongyongjiao/wx205235 20150815 begin */
    fpc_log_info("fpc1020 index= %d - xpos= %d - ypos= %d \n", index, xpos, ypos);
    /* DTS2015081500317 tongyongjiao/wx205235 20150815 end > */

    for (x = 0; x < xSubareaNum; x++) {
        for (y = 0; y < ySubareaNum; y++) {
            /* < DTS2015050502100 tongyongjiao/wx205235 20150505 begin */
            if (xpos >= xp[x] && xpos < (xp[x] + 8) && ypos >= yp[y] && ypos < (yp[y] + 8)) {
            /* DTS2015050502100 tongyongjiao/wx205235 20150505 end > */
...
/* DTS2015090705390 tongyongjiao/wx205235 20150907 end > */
...
/* < DTS2015031001664 caiying/00214377 20150310 begin */
/* < DTS2015061004109 tongyongjiao/wx205235 20150610 begin */
...
/* DTS2015061004109 tongyongjiao/wx205235 20150610 end > */
/* DTS2015031001664 caiying/00214377 20150310 end > */
...
static int fpc1020_check_for_deadPixels(fpc1020_data_t *fpc1020, u8* ripPixels, bool bCB)
{
    ...
    /* < DTS2015071702807 zhangkailiang/00349567 20150717 begin */
    /* DTS2015071702807 zhangkailiang/00349567 20150717 end > */
...
/* -------------------------------------------------------------------- */
/* DTS2015051105241 cheyufeng  20150511 end> */
/* DTS2015042903888 LiuQiang   20150429 end> */
