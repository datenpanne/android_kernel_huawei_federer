...
#ifdef PRODUCTION_PLE_KERNEL
#define CUTOFF_VOLTAGE_UV 3300000
#else
...
/* < DTS2015021201721 chengkai/wx209700 20150319 begin */
#define SOC_99		99
/* DTS2015021201721 chengkai/wx209700 20150319 end> */
/* < DTS2015042502343 yuanzhen 20150428 begin */
/* < DTS2015070304346 taohanwen 20150716 begin */
#define RESUME_SOC_CHANGE_MAX	1
/* DTS2015070304346 taohanwen 20150716 end > */
/* DTS2015042502343 yuanzhen 20150428 end > */
/* < DTS2015041608371 fangqinwu 20150515 begin */
#define SOC_ONE		1
/* DTS2015041608371 fangqinwu 20150515 end > */
#endif
...
#endif
/* DTS2015032004578 chengkai/wx209700 20150331 end> */
...
/* DTS2014070701815 jiangfei 20140707 end> */
	...
	/* <DTS2014081202785 sunwenyong 20140818 begin */
	/* <DTS2015032804712 taohanwen 20150505 begin */
#ifdef CONFIG_HUAWEI_KERNEL
	struct work_struct      set_batt_status_work;
	int						vbat_store_uv[VAVG_MAX_COUNT];
#endif
	/* DTS2015032804712 taohanwen 20150505 end> */
	/* DTS2014081202785 sunwenyong 20140818 end> */
...
/* DTS2014080100080  mapengfei 20140805 end> */
    ...
    /* DTS2014071002612  mapengfei 20140710 end> */
	/* < DTS2015070304346 taohanwen 20150716 begin */
	/* < DTS2015091806058 taohanwen 20150916 begin*/
#ifdef CONFIG_HUAWEI_KERNEL
	struct delayed_work		report_soc_work;
	bool					resume_soc_work;
#endif
	/* DTS2015091806058 taohanwen 20150916 end > */
	/* DTS2015070304346 taohanwen 20150716 end > */
};

static struct qpnp_bms_chip *the_chip;

/* <DTS2015061809777 mapengfei 20150620 begin */
#ifdef CONFIG_HUAWEI_KERNEL
static int bms_capacity=0;
static int real_soc=0;
#endif
/* DTS2015061809777 mapengfei 20150620 end> */
/*
...
static int get_battery_status(struct qpnp_bms_chip *chip)
{
	...
	if (chip->batt_psy) {
		...
		if(!chip->use_ext_charger){
          ...
          /* <DTS2015061809777 mapengfei 20150615 begin */
			/* <DTS2015012706792 jiangfei 20150205 begin */
			ret.intval = hw_get_battery_status();
			/* DTS2015012706792 jiangfei 20150205 end> */
         /* DTS2015061809777 mapengfei 20150615 end> */
		}
#else
...
/* DTS2015032004578 chengkai/wx209700 20150331 end> */
    ...
    /* < DTS2015061809777 mapengfei 20150616 begin */
#ifdef CONFIG_HUAWEI_KERNEL
    real_soc = chip->calculated_soc;
#endif
    /* DTS2015061809777 mapengfei 20150616 end > */
...
static int report_state_of_charge(struct qpnp_bms_chip *chip)
{
...
#ifdef CONFIG_HUAWEI_KERNEL
	/* < DTS2015072004746 taohanwen 20150720 begin */
	{
		if (chip->ui_soc_tag) {
			soc = chip->ui_soc;
		} else {
	...
	/* DTS2015072004746 taohanwen 20150720 end > */
#else
		soc = report_vm_bms_soc(chip);
#endif
	/* <DTS2015061809777  mapengfei 20150611 begin */
#ifdef CONFIG_HUAWEI_KERNEL
	...
	bms_capacity=soc;
#endif
	/* DTS2015061809777  mapengfei 20150611 end> */
	/* DTS2015070304346 taohanwen 20150716 end > */
	mutex_unlock(&chip->last_soc_mutex);
...
/* <DTS2015061809777 mapengfei 20150612 begin */
#ifdef CONFIG_HUAWEI_KERNEL
...
#endif
/* DTS2015061809777 mapengfei 20150612 end> */
/* <DTS2014081304095 sunwenyong 20140815 begin */
#ifdef PRODUCTION_PLE_KERNEL
#define HW_PROTECT_VOLTAGE_UV 3300000
#else
#define HW_PROTECT_VOLTAGE_UV 3400000
#endif
...
int hw_discharge_voltage_check(struct qpnp_bms_chip * chip, int soc, int vbat_uv, int batt_temp)
{
	...
	return soc;
	/* DTS2015021007448 jiangfei 20150216 end> */
}
...
/* <DTS2015032004020 jiangfei 20150320 begin */
#ifdef CONFIG_HUAWEI_PMU_DSM
/* <DTS2015070301703 guochenguang 20150709 begin */
#define POWERON_SOC_CHECK_INVALID_SECOND (120)
/* DTS2015070301703 guochenguang 20150709 end> */
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	if (chip->ui_soc_tag) {
		if (chip->charger_removed_since_full && !chip->charger_reinserted) {
            ...
            {
                /* < DTS2015042208779 chendeng 20150422 begin */
                /* Add check the last_soc value. */
                if ((chip->ui_soc > chip->last_soc)
                    && (chip->last_soc != -EINVAL))
                /* DTS2015042208779 chendeng 20150422 end> */
                {
                    if(chip->ui_soc_high_current)
                    {
                        chip->ui_soc--;
...
#ifdef CONFIG_HUAWEI_KERNEL
			/* < DTS2014112106390 chenyuanquan 20141121 begin */
			batt_data = &palladium_1500_data;
			/* DTS2014112106390 chenyuanquan 20141121 end > */
			goto assign_data;
#else
			return -EINVAL;
#endif
