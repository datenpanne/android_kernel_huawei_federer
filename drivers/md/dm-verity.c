...
out:
	...
	{
	DMERR_LIMIT("%s: %s block %llu is corrupted[logging]", v->data_dev->name,  type_str, block);

		return 0;
	}
	if (v->mode == DM_VERITY_MODE_RESTART)
	{
   DMERR_LIMIT("%s: %s block %llu is corrupted[restart]", v->data_dev->name,  type_str, block); 		
